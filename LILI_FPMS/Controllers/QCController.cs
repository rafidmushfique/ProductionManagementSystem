...
namespace LILI_FPMS.Controllers
{
    ...
    public class QCController : Controller
    {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            var qc_data = from master in _context.TblQc
                          from requisition in _context.TblProcessOrder
                          from process in _context.TblProductionProcess
                          from bom in _context.View_BOM
                          from p in _context.View_Product
                          where(master.LinkedProcessNo == requisition.ProcessOrderNo && master.ProcessNo == process.ProcessNo && requisition.ProductCode == bom.ProductCode && bom.ProductCode== p.ProductCode)
                          select new TblQc {
                              Id = master.Id,
        ...
        //[Bind("EnrollmentDate,FirstMidName,LastName")] Student student)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (_context.TblQc.Any(x => x.Qcno == qc_data.Qcno))
                    {
                ...
                var fgCodeQuery = from p in _context.TblProductionProcess
                             join r in _context.TblProcessOrder on p.LinkedProcessNo equals r.ProcessOrderNo
        ...
        public ActionResult Update(int id)
        {
            ...
            var productCode =  _context.TblProcessOrder.Where(req => req.ProcessOrderNo == dt.LinkedProcessNo).FirstOrDefault().ProductCode;
            ...
            //from qc in _context.TblQc
            //from qcd in _context.TblQcdetails
            ...
            var qc_detail = from qc in _context.TblQc
                            from qcd in _context.TblQcdetails
                            from qcp in _context.TblQcparameter
                            from po in _context.TblProcessOrder
                            where qc.Qcno == qcd.Qcno && qcp.QcparameterCode == qcd.QcparameterCode && qc.LinkedProcessNo == po.ProcessOrderNo && qc.Qcno == dt.Qcno
        ...
        public IActionResult addProcessOrder()
        {
            ...
            var model = _context.GetSearchProcessOrderList
                               .FromSql("EXEC sp_SearchProcessOrderForQC @processOrderNo", processOrderNoParam)
                               .ToList();
        ...
        public JsonResult GetProcessNumber(string RequisitionNo)
        {
            var model = new List<TblProductionProcess>(from prop in _context.TblProductionProcess
                                                       from req in _context.TblProcessOrder
        ...
        public JsonResult SetRequisitionInfomation(string ProcessOrderNo)
        {
            ...
            if (ProcessOrderNo != "")
            {
                var availableFloorStock = from c in _context.TblProcessOrder
                ...
                var sa = new JsonSerializerSettings();
                var expertiesInfo = from c in _context.TblProcessOrder
                                    from b in _context.View_BOM
                                    from p in _context.View_Product
                                    where(c.ProcessOrderNo == ProcessOrderNo && c.ProductCode == b.ProductCode && b.ProductCode == p.ProductCode)
                                    select new
                                    {
                                        c.ProcessOrderNo,
                                        c.BatchNo,
