...
namespace LILI_FPMS.Controllers
{
    ...
    public class ProcessOrderController : Controller
    {
        ...
        public ProcessOrderController(dbFormulationProductionSystemContext context)
        {
            _context=context;
        }
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
           // TblProcessOrder req= new TblProcessOrder();
            ...
            ViewData["CurrentFilter"] = searchString;
             ...
             var req = from s in _context.TblProcessOrder
            ...
            int pageSize = 10;
            return View(await PaginatedList<TblProcessOrder>.CreateAsync(req.AsNoTracking(), pageNumber ?? 1, pageSize));
        ...
        public ActionResult Create()
        {
            ...
            TblProcessOrder entities = new TblProcessOrder();
            entities.ProcessOrderNo = processOrderNo;
            entities.ProcessOrderDate = DateTime.Now;
            return View(entities);
        }

        public async Task<IActionResult> CreateProcessOrder(TblProcessOrder model)
        {
            try
            {
                if (ModelState.IsValid) { 
                    ...
                    if (model.LineCode != null)
                    {
                        ...
                        {
                            TblRequisitionLine rl = new TblRequisitionLine();
                            rl.ProcessOrderNo = model.ProcessOrderNo;
                            rl.LineCode = item;
                            model.TblRequisitionLine.Add(rl);
                        }
                    }
        ...
        public ActionResult Update(int id)
        {
            ...
            TblProcessOrder model= new TblProcessOrder();
            var dt =_context.TblProcessOrder.Where(s=>s.Id== id).First();
            model.Id = dt.Id;
            ...
            model.BatchSize = _context.View_BOM.Where(s => s.ProductCode == dt.ProductCode).FirstOrDefault().BatchSize;
            model.StandardOutput = _context.View_BOM.Where(s => s.ProductCode == dt.ProductCode).FirstOrDefault().StandardOutput;
            ...
            model.TblProcessOrderDetail = _context.TblProcessOrderDetail.Where(d => d.ProcessOrderNo == dt.ProcessOrderNo).ToList();
            model.Bomid = dt.Bomid;
            model.ManPower=dt.ManPower;

            var processDetails= from dtl in _context.TblProcessOrderDetail
                                ...
                                select new TblProcessOrderDetail
                                {
                                    Id = dtl.Id,
           ...
           // var selectedMachine = _context.TblRequisitionMachineSetup.Where(s => s.ProcessOrderNo == dt.ProcessOrderNo).Select(x => x.MachineCode).ToList();
            var selectedLine = _context.TblRequisitionLine.Where(s => s.ProcessOrderNo == dt.ProcessOrderNo).Select(x => x.LineCode).ToList();
            ...
            model.TblProcessOrderDetail = processDetails.ToList();
        ...
        public async Task<IActionResult> UpdateProcessOrder(int id, TblProcessOrder model)
        {
            var checkIfExists = _context.TblProcessOrder.Any(s => s.Id == id);
            if (!checkIfExists)
            {
                return NotFound();
            }
            try
            {
                ...
                var processOrderToUpdate = await _context.TblProcessOrder.FirstOrDefaultAsync(s => s.Id == id);


                var requistionNo= _context.TblRequisition.Where(s=>s.Id==id).Select(x=> x.RequisitionNo).FirstOrDefault();

                if (IsEditableDeleteable(requistionNo) == true)
                {
                    ...
                    _context.TblProcessOrderDetail.RemoveRange(_context.TblProcessOrderDetail.Where(d => d.ProcessOrderNo == model.ProcessOrderNo));

                    if (model.TblProcessOrderDetail != null)
                    {
                        foreach (var item in model.TblProcessOrderDetail.ToList())
                        {
                            TblProcessOrderDetail proDetail = new TblProcessOrderDetail();
        ...
        public bool Delete(int id)
        {
            try
            {
                ...
                TblProcessOrder req = _context.TblProcessOrder.Where(s => s.Id == id).First();
                var requisitionNo= _context.TblRequisition.Where(s=> s.LinkedProcessNo == req.ProcessOrderNo).Select(x=> x.RequisitionNo).FirstOrDefault();
                if (IsEditableDeleteable(requisitionNo) == true)
                {
                    _context.TblRequisitionLine.RemoveRange(_context.TblRequisitionLine.Where(d => d.ProcessOrderNo == req.ProcessOrderNo));
                    _context.TblProcessOrderDetail.RemoveRange(_context.TblProcessOrderDetail.Where(d => d.ProcessOrderNo == req.ProcessOrderNo));
                    _context.TblProcessOrder.Remove(req);
        ...
        private string GenerateProcessOrderNo() {
            ...
            var processOrderNo = "PON-" + yy + mn;


            String maxId = "";
            String maxNo = (from c in _context.TblProcessOrder.Where(c => c.ProcessOrderNo.Substring(0, 8) == processOrderNo).OrderByDescending(t => t.Id) select c.ProcessOrderNo.Substring(8, 3)).FirstOrDefault();
            ...
            processOrderNo = "PON-" + yy + mn + maxId;

            return processOrderNo;
        }
        private bool DoesPorcessOrderNoExists(string vProcessOrderNo)
        { 
         return _context.TblProcessOrder.Any(e => e.ProcessOrderNo == vProcessOrderNo);
        }
    }
}
