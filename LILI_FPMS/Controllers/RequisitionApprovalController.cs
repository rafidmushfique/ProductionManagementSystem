        ...
        public IActionResult RequisitionApprovalStatus(string requisitionNo, string approvalStatus)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    string queryDel = "DELETE TblRequisitionApprovalStatus WHERE RequisitionNo =" + "'" + ras.RequisitionNo + "'";
                    SqlCommand cmdDel = new SqlCommand(queryDel, con);
                    con.Open();
                    int d = cmdDel.ExecuteNonQuery();
                    con.Close();

                    string query = "INSERT INTO TblRequisitionApprovalStatus(RequisitionNo, RequisitionStatusDate, ApprovalStatus, Approver) " +
